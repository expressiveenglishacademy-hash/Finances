...
async function getData() {
  ...
  try {
    ...
    const localHasBusinessData = hasBusinessData(localData);
    const remoteHasBusinessData = hasBusinessData(remoteData);

    let workingData = remoteHasBusinessData ? remoteData : localData;

    if (localHasBusinessData) {
      const mergedData = mergeDataSets(remoteData, localData);
      await syncFullDataToSupabase(mergedData);
      workingData = await fetchAllDataFromSupabase();
    } else if (remoteHasBusinessData) {
      workingData = remoteData;
    } else {
      await syncFullDataToSupabase(localData);
      workingData = await fetchAllDataFromSupabase();
    }

    remoteSyncReady = true;
    saveData(workingData, { skipRemote: true });
    return workingData;
  } catch (error) {
    console.error("Supabase sync failed, using local fallback:", error);
    remoteSyncReady = true;
    return localData;
  }
}
...
function queueRemoteSync(data) {
  ...
  remoteSyncTimer = window.setTimeout(() => {
    remoteSyncPromise = remoteSyncPromise
      .then(() => syncFullDataToSupabase(snapshot))
      .catch((error) => {
...
function hasBusinessData(data) {
  return Boolean(
    ...
    (data.teacherPayments && data.teacherPayments.length) ||
    (data.investments && data.investments.length)
  );
}

function mergeDataSets(remoteData, localData) {
  const merged = {
    ...remoteData,
    ...
    expenses: mergeRecordsByKey(remoteData.expenses, localData.expenses, (item) => item.id),
    teacherPayments: mergeRecordsByKey(remoteData.teacherPayments, localData.teacherPayments, (item) => item.id),
    investments: mergeRecordsByKey(remoteData.investments, localData.investments, (item) => item.id)
  };
...
function mergeRecordsByKey(first = [], second = [], keyBuilder) {
  const map = new Map();
...
async function fetchAllDataFromSupabase() {
  const [
    ...
    supabaseClient.from("teacher_payments").select("*").order("created_at", { ascending: false }),
    supabaseClient.from("investments").select("*").order("created_at", { ascending: false }),
    supabaseClient.from("accounts").select("*").order("created_at", { ascending: true }),
    supabaseClient.from("app_settings").select("*").order("id", { ascending: true }).limit(1)
  ]);
  ...
  const settingsRow = settingsResponse.data && settingsResponse.data.length ? settingsResponse.data[0] : null;
  const defaults = createEmptyData();

  return {
    incomeCategories: Array.isArray(settingsRow?.income_categories) ? settingsRow.income_categories : defaults.incomeCategories,
    expenseCategories: Array.isArray(settingsRow?.expense_categories) ? settingsRow.expense_categories : defaults.expenseCategories,
    investmentCategories: Array.isArray(settingsRow?.investment_categories) ? settingsRow.investment_categories : defaults.investmentCategories,
    accounts: (accountsResponse.data || []).map(mapAccountRowToApp),
    students: (studentsResponse.data || []).map(mapStudentRowToApp),
    teachers: normalizeTeachers((teachersResponse.data || []).map(mapTeacherRowToApp)),
    incomes: normalizeLegacyTimestamps((incomesResponse.data || []).map(mapIncomeRowToApp), "income"),
    expenses: normalizeLegacyTimestamps((expensesResponse.data || []).map(mapExpenseRowToApp), "expense"),
    teacherPayments: normalizeLegacyTimestamps((teacherPaymentsResponse.data || []).map(mapTeacherPaymentRowToApp), "teacher"),
    investments: normalizeLegacyTimestamps((investmentsResponse.data || []).map(mapInvestmentRowToApp), "investment")
  };
}

async function syncFullDataToSupabase(data) {
  await upsertRows("students", (data.students || []).map(mapStudentAppToRow));
  await upsertRows("teachers", normalizeTeachers(data.teachers || []).map(mapTeacherAppToRow));
  await upsertRows("accounts", (data.accounts || []).map(mapAccountAppToRow));
  await upsertRows("incomes", (data.incomes || []).map(mapIncomeAppToRow));
  await upsertRows("expenses", (data.expenses || []).map(mapExpenseAppToRow));
  await upsertRows("teacher_payments", (data.teacherPayments || []).map(mapTeacherPaymentAppToRow));
  await upsertRows("investments", (data.investments || []).map(mapInvestmentAppToRow));

  await deleteMissingRows("students", data.students || []);
  await deleteMissingRows("teachers", data.teachers || []);
  await deleteMissingRows("accounts", data.accounts || []);
  await deleteMissingRows("incomes", data.incomes || []);
  await deleteMissingRows("expenses", data.expenses || []);
  await deleteMissingRows("teacher_payments", data.teacherPayments || []);
  await deleteMissingRows("investments", data.investments || []);

  await saveSettingsToSupabase(data);
}
...
async function deleteMissingRows(table, items) {
  const localIds = new Set((items || []).map((item) => item.id).filter(Boolean));
...
async function saveSettingsToSupabase(data) {
  ...
  const payload = {
    income_categories: data.incomeCategories || ["Mensualidad"],
    expense_categories: data.expenseCategories || [],
    investment_categories: data.investmentCategories || []
  };
...
async function fetchStudentsFromSupabase() {
  ...
  if (error) throw error;
  return (data || []).map(mapStudentRowToApp);
}

async function syncLocalStudentsToSupabase(students) {
...
async function upsertStudentToSupabase(student) {
  const { error } = await supabaseClient
    .from("students")
    .upsert(mapStudentAppToRow(student), { onConflict: "id" });
...
function mapStudentAppToRow(student) {
  return {
    id: student.id,
    name: student.name || "",
    level: student.level || "",
    ...
    material_fee: Number(student.materialFee || 0),
    material_currency: student.materialCurrency || "NIO",
    enrollment_date: student.enrollmentDate || null
  };
}

function mapTeacherRowToApp(row) {
  return {
    id: row.id || cryptoRandom(),
    name: row.name || "",
    contact: row.contact || "",
...
function mapTeacherAppToRow(teacher) {
  return {
    id: teacher.id,
    name: teacher.name || "",
...
function mapAccountRowToApp(row) {
  return {
    id: row.id || cryptoRandom(),
    name: row.name || "",
    type: row.type || "",
...
function mapAccountAppToRow(account) {
  return {
    id: account.id,
    name: account.name || "",
    type: account.type || "",
...
function mapIncomeRowToApp(row) {
  return {
    id: row.id || cryptoRandom(),
    ...
    student: row.student || "",
    level: row.level || "",
    receiptLevel: row.receipt_level || row.level || "",
    concept: row.concept || "",
    category: row.category || "Mensualidad",
    method: row.method || "",
...
function mapIncomeAppToRow(item) {
  return {
    id: item.id,
    ...
    level: item.level || "",
    receipt_level: item.receiptLevel || item.level || "",
    concept: item.concept || "",
    category: item.category || "Mensualidad",
    method: item.method || "",
    currency: item.currency || "USD",
    exchange_rate: Number(item.exchangeRate || 1),
    original_amount: Number(item.originalAmount ?? item.amount ?? 0),
...
function mapExpenseRowToApp(row) {
  return {
    id: row.id || cryptoRandom(),
    date: row.date || todayValue(),
    createdAt: row.created_at || new Date().toISOString(),
...
function mapExpenseAppToRow(item) {
  return {
    id: item.id,
    date: item.date || todayValue(),
    created_at: item.createdAt || new Date().toISOString(),
...
function mapTeacherPaymentRowToApp(row) {
  return {
    id: row.id || cryptoRandom(),
    teacher: row.teacher || "",
    date: row.date || todayValue(),
    createdAt: row.created_at || new Date().toISOString(),
    subcategory: row.subcategory || "Horas laborales regulares",
    description: row.description || buildTeacherPaymentDescription(row.teacher, row.subcategory),
    currency: row.currency || "USD",
    exchangeRate: Number(row.exchange_rate || 1),
    originalAmount: Number(row.original_amount ?? row.amount ?? 0),
    amount: Number(row.amount || 0),
...
function mapTeacherPaymentAppToRow(item) {
  return {
    id: item.id,
    ...
    created_at: item.createdAt || new Date().toISOString(),
    subcategory: item.subcategory || "Horas laborales regulares",
    description: item.description || buildTeacherPaymentDescription(item.teacher, item.subcategory),
    currency: item.currency || "USD",
    exchange_rate: Number(item.exchangeRate || 1),
    original_amount: Number(item.originalAmount ?? item.amount ?? 0),
    amount: Number(item.amount || 0),
...
function mapInvestmentRowToApp(row) {
  return {
    id: row.id || cryptoRandom(),
    date: row.date || todayValue(),
    createdAt: row.created_at || new Date().toISOString(),
...
function mapInvestmentAppToRow(item) {
  return {
    id: item.id,
    date: item.date || todayValue(),
    created_at: item.createdAt || new Date().toISOString(),
...
function renderStudentsPage(data) {
  ...
  if (form) {
    form.addEventListener("submit", async (event) => {
      ...
      try {
        await upsertStudentToSupabase(payload);
      } catch (error) {
        console.error(error);
        toast("No se pudo guardar en la nube. Revisa Supabase.");
        return;
      }
    ...
    summaryCards.innerHTML = stats.details.length
      ? stats.details.slice(0, 5).map((student) => `
        <div class="mini-stat-card">
          <span>${escapeHtml(student.name)}</span>
          <strong>${student.statusPayment}</strong>
          <small>�ltimo pago: ${student.lastPayment ? formatDate(student.lastPayment) : "Sin pago"}</small>
          <small>Due date actual: d�a ${student.dueDay}</small>
        </div>
      `).join("")
      : emptyMessage("A�n no hay estudiantes registrados.");
  }
  ...
  document.querySelectorAll("[data-edit-due-date]").forEach((button) => {
    button.onclick = async () => {
      ...
      const newDueDay = window.prompt(
        `Ingresa el nuevo due date para ${student.name} (d�a del 1 al 31):`,
      ...
      if (!Number.isInteger(parsedDay) || parsedDay < 1 || parsedDay > 31) {
        toast("Debes ingresar un d�a v�lido entre 1 y 31.");
        return;
      }
      ...
      try {
        ...
        console.error(error);
        toast("No se pudo actualizar en la nube.");
        return;
      }
...
function bindEditStudentButtons(data) {
  document.querySelectorAll("[data-edit-student]").forEach((button) => {
    button.onclick = async () => {
      ...
      if (!cleanName) {
        toast("El nombre no puede quedar vac�o.");
        return;
      }
      ...
      if (!Number.isInteger(parsedDueDay) || parsedDueDay < 1 || parsedDueDay > 31) {
        toast("El due date debe ser un n�mero entre 1 y 31.");
        return;
      }
  ...
  document.querySelectorAll("[data-delete-student]").forEach((button) => {
    button.onclick = async () => {
      ...
      const confirmed = window.confirm(
        `Se eliminar� el estudiante ${student.name}. �Deseas continuar?`
      );
      if (!confirmed) return;
...
function renderEnrollmentPage(data) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (studentType === "nino" && (!motherName || !guardianPhone)) {
      toast("Para estudiantes ni�os debes agregar nombre de la mam� y tel�fono.");
      return;
    }
    ...
    try {
      ...
      console.error(error);
      toast("No se pudo guardar la matr�cula en la nube.");
      return;
    }
    ...
    toast("Matr�cula guardada correctamente. El estudiante ya aparece en la secci�n Estudiantes.");
  ...
  if (downloadButton) {
    downloadButton.addEventListener("click", () => {
      if (!latestEnrollmentReceipt) {
        toast("Primero genera una matr�cula para descargar el comprobante.");
        return;
      }
